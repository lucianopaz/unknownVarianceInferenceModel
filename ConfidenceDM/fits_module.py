...
def main(task=1,ntasks=1, task_base=1, method='full', optimizer='cma',
         save=False, show=False, fit=True, suffix='', rt_cutoff=None, 
         merge=None, fixed_parameters={}, dmKwargs={}, start_point={},
         bounds={}, optimizer_kwargs={}, experiment='all', debug=False,
         confidence_partition=100, plot_merge=None, verbose=False,
         save_plot_handler=False, load_plot_handler=False, 
         start_point_from_fit_output=None, override=False,
         plot_handler_rt_cutoff=None, confidence_mapping_method='log_odds',
         plot_binary=None, save_stats=False, binary_split_method='median',
         fits_path='fits_cognition'):
    
    # Prepare subjectSessions list
    subjects = io.filter_subjects_list(io.unique_subject_sessions(),
                                       'all_sessions_by_experiment')
    if experiment!='all':
        subjects = io.filter_subjects_list(subjects,'experiment_'+experiment)
    package_logger.debug('Total number of subjectSessions listed = {0}'.\
                         format(len(subjects)))
    package_logger.debug('Total number of subjectSessions that will be fitted '
                         '= {0}'.format(len(range(task,len(subjects),ntasks))))
    ...
    for i,s in enumerate(subjects):
        ...
        if (i-task)%ntasks==0:
            package_logger.info('Task will execute for enumerated {0} subject '
                                '{1}'.format(i,s.get_key()))
            ...
            if fit:
                ...
                if override or not (os.path.exists(fname) and
                    ...
                    if start_point_from_fit_output:
                        package_logger.debug('Flag start_point_from_fit_output '
                                             'was present. Will load '
                        ...
                        loaded_cmapmeth = start_point_from_fit_output['cmapmeth']
                        fname = Fitter_filename(experiment=s.experiment,
                                                method=loaded_method,
                                                name=s.get_name(),
                                                session=s.get_session(),
                                                optimizer=loaded_optimizer,
                                                suffix=loaded_suffix,
                                                confidence_map_method=loaded_cmapmeth,
                                                fits_path=fits_path)
                        package_logger.debug('Will load parameters from '
                                             'file: {0}'.format(fname))
                        fixed_parameters,start_point = prepare_fit_args(fitter,
                                                                        options,
                                                                        fname)
                    else:
                    ...
                    package_logger.warning('File {0} already exists, will skip '
                                           'enumerated subject {1} whose key '
                                           'is {2}. If you wish to override '
                                           'saved Fitter instances, supply the '
                                           'flag -w.'.format(fname,i,s.get_key()))
            ...
            if save_stats:
                fname = Fitter_filename(experiment=s.experiment,
                                        ...
                                        optimizer=optimizer,
                                        suffix=suffix,
                                        confidence_map_method=high_confidence_mapping_method,
                                        fits_path=fits_path)
                package_logger.debug('save_stats flag was true. Will load '
                                     'Fitter instance from file {0} and '
                                     'compute stats.'.format(fname))
                # Try to load the fitted data from file 'fname' or continue to next subject
                try:
                    fitter = load_Fitter_from_file(fname)
                    loaded_fitter = True
                except:
                    package_logger.warning('Failed to load fitter from file '
                                           '{0}. Will continue to next '
                                           'subject.'.format(fname))
                    loaded_fitter = False
                if loaded_fitter:
                    ...
                    if override or not (os.path.exists(fname) and
                        ...
                        with open(fname,'w') as f:
                            package_logger.debug('Computing stats and storing '
                                                 'them in file "{0}".'.\
                                                 format(fname))
                            stats = fitter.stats(return_mean_rt=True,
                        ...
                        package_logger.warning('File {0} already exists, will '
                                               'skip stats computation for '
                                               'enumerated subject {1} whose '
                                               'key is {2}. If you wish to '
                                               'override saved a Fitter '
                                               'instance stats, supply the '
                                               'flag -w.'.format(fname, i,
                                                                 s.get_key()))
            
            # Prepare plotable data
            if show or save or save_plot_handler:
                package_logger.debug('show, save or save_plot_fitter flags '
                                     'were True.')
                if load_plot_handler:
                    fname = Fitter_filename(experiment=s.experiment,
                                            ...
                                            optimizer=optimizer,
                                            suffix=suffix,
                                            confidence_map_method=confidence_mapping_method,
                                            fits_path=fits_path).\
                                                replace('.pkl',
                                                        '_plot_handler.pkl')
                    package_logger.debug('Loading Fitter_plot_handler from '
                                         'file={0}'.format(fname))
                    try:
                        f = open(fname, 'r')
                        temp = pickle.load(f)
                        ...
                        package_logger.warning('Failed to load '
                                               'Fitter_plot_handler from '
                                               'file={0}. Will continue to '
                                               'next subject.'.format(fname))
                        continue
                else:
                    fname = Fitter_filename(experiment=s.experiment,
                                            method=method,
                    ...
                    fitter = load_Fitter_from_file(fname)
                    try:
                        package_logger.debug('Attempting to load fitter from '
                                             'file "{0}".'.format(fname))
                        fitter = load_Fitter_from_file(fname)
                    except:
                        package_logger.warning('Failed to load fitter from '
                                               'file {0}. Will continue to '
                                               'next subject.'.format(fname))
                        continue
                    # Create Fitter_plot_handler for the loaded Fitter instance
                    package_logger.debug('Getting Fitter_plot_handler with '
                                         'merge_plot={0}.'.format(plot_merge))
                    if not plot_handler_rt_cutoff is None:
                        if s.experiment=='Luminancia':
                            cutoff = np.min([1.,plot_handler_rt_cutoff])
                        else:
                            cutoff = plot_handler_rt_cutoff
                        package_logger.debug('Fitter_plot_handler will use '
                                             'rt_cutoff = {0}'.format(cutoff))
                        edges = np.linspace(0, cutoff, 51)
                    else:
                        edges = None
                    temp = fitter.get_fitter_plot_handler(merge=plot_merge,
                                                          edges=edges)
                    if save_plot_handler:
                        fname = fname.replace('.pkl','_plot_handler.pkl')
                        if override or not (os.path.exists(fname) and
                                            os.path.isfile(fname)):
                            package_logger.debug('Saving Fitter_plot_handler '
                                                 'to file={0}.'.format(fname))
                            temp.save(fname)
                        else:
                            package_logger.warning('Could not save '
                                                   'Fitter_plot_handler. File '
                                                   '{0} already exists. To '
                                                   'override supply the flag '
                                                   '-w.'.format(fname))
    ...
    if save:
        if task==0 and ntasks==1:
            fname = "fits_{experiment}{method}_{cmapmeth}{suffix}".\
                    format(experiment=experiment+'_' if experiment!='all' else '',
                           method=method,
                           suffix=suffix,
                           cmapmeth=confidence_mapping_method)
        else:
            fname = "fits_{experiment}{method}_{cmapmeth}_{task}_{ntasks}"\
                    "{suffix}".\
                    format(experiment=experiment+'_' if experiment!='all' else '',
                           method=method,
                           task=task,
    ...
    if show or save:
        ...
        assert not fitter_plot_handler is None, 'Could not create the '\
            'Fitter_plot_handler to plot the fitter results'
        if plot_merge and load_plot_handler:
            fitter_plot_handler = fitter_plot_handler.merge(plot_merge)
        fitter_plot_handler.plot(saver=saver,
                                 show=show,
                                 is_binary_confidence=plot_binary,
                                 binary_split_method=binary_split_method)
        if save:
